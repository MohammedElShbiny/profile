...
function getParam(url, segment) {
  const parts = new URL(url, 'http://localhost').pathname.split('/');
  const idx = parts.indexOf(segment);
  if (idx >= 0 && idx + 1 < parts.length) return decodeURIComponent(parts[idx + 1]);
  return null;
}
...
function checkAuth(req) {
  ...
  return user === (process.env.ADMIN_USER || 'admin') && pass === (process.env.ADMIN_PASS || 'demo2024');
}
...
async function seedIfEmpty() {
  ...
  const projects = [
    { title_en: 'Quantum Dashboard', title_ar: 'لوحة كم الحوسبة', description_en: 'Real-time analytics dashboard with AI-powered insights and predictive modeling.', description_ar: 'لوحة تحليلات فورية مع رؤى مدعومة بالذكاء الاصطناعي ونمذجة تنبؤية.', status: 'live', gradient: 'linear-gradient(135deg,#0070f3 0%,#7759fb 100%)', icon: 'dashboard', tech_stack: JSON.stringify(['React', 'TypeScript', 'Node.js', 'AI']), github_url: '#', demo_url: '#', sort_order: 1 },
    { title_en: 'SyncFit Pro', title_ar: 'سينك فيت برو', description_en: 'Cross-platform fitness tracking app with AI workout recommendations and progress analytics.', description_ar: 'تطبيق لياقة بدنية متعدد المنصات مع توصيات تمارين بالذكاء الاصطناعي.', status: 'live', gradient: 'linear-gradient(135deg,#00b954 0%,#4ae176 100%)', icon: 'sync', tech_stack: JSON.stringify(['Android', 'Kotlin', 'Node.js']), github_url: '#', demo_url: '#', sort_order: 2 },
    { title_en: 'Forge Architect', title_ar: 'فورج أركيتيكت', description_en: 'Restaurant management system with real-time inventory tracking and AI-powered ordering.', description_ar: 'نظام إدارة مطاعم مع تتبع المخزون الفوري والطلبات بالذكاء الاصطناعي.', status: 'maintenance', gradient: 'linear-gradient(135deg,#512bd4 0%,#c9beff 100%)', icon: 'architecture', tech_stack: JSON.stringify(['React', 'Node.js', 'AI']), github_url: '#', demo_url: '#', sort_order: 3 },
    { title_en: 'Aether API Gateway', title_ar: 'إيثر بوابة API', description_en: 'Scalable API gateway with intelligent rate limiting, caching, and real-time monitoring.', description_ar: 'بوابة API قابلة للتوسع مع تحديد المعدل الذكي والتخزين المؤقت والمراقبة الفورية.', status: 'live', gradient: 'linear-gradient(135deg,#0070f3 0%,#4ae176 100%)', icon: 'cloud', tech_stack: JSON.stringify(['Node.js', 'TypeScript', 'AI']), github_url: '#', demo_url: '#', sort_order: 4 },
  ];
  for (const p of projects) {
    await db.execute({
      sql: `INSERT INTO projects (title_en, title_ar, description_en, description_ar, status, gradient, icon, tech_stack, github_url, demo_url, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [p.title_en, p.title_ar, p.description_en, p.description_ar, p.status, p.gradient, p.icon, p.tech_stack, p.github_url, p.demo_url, p.sort_order],
    });
  }
  ...
  for (const s of services) {
    await db.execute({
      sql: `INSERT INTO services (name_en, name_ar, price_egp, description_en, description_ar, features_en, features_ar, is_popular, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [s.name_en, s.name_ar, s.price_egp, s.description_en, s.description_ar, s.features_en, s.features_ar, s.is_popular, s.sort_order],
    });
  }
}

module.exports = { getClient, initDb, seedIfEmpty, checkAuth, jsonRes, errRes, corsPreflight, parseBody, parseQuery, getParam };
